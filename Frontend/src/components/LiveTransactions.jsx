    ...
    axios.get(`${import.meta.env.VITE_API_URL}/trades/artist/${artistId}`, {
      ...
      .then(response => {
        ...
        const formattedTrades = response.data.map(trade => ({
          ...
          amountInUsd: new BigNumber(trade.amountInUsd || 0).toFixed(2),
          priceInUsd: new BigNumber(trade.priceInUsd || 0).toFixed(8),
    ...
    client.onConnect = (frame) => {
      ...
      const subscription = client.subscribe(`/topic/trades/${artistId}`, (message) => {
        ...
        try {
          ...
          console.log("[LiveTransactions] RAW TRADE FROM WS:", trade);
          console.log("[LiveTransactions] timestamp field:", trade.timestamp);
          console.log("[LiveTransactions] timestamp type:", typeof trade.timestamp);
