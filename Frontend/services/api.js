...
// Update artist contract address
export const updateArtistContractAddress = async (spotifyId, artistDetails) => {
  try {
    const jwtToken = getAuthToken();
    if (!jwtToken) {
      throw new Error("JWT token missing. User might not be logged in.");
    ...
    const payload = {
      contractAddress: artistDetails.contractAddress || artistDetails,
    };

    console.log("[updateArtistContractAddress] Updating contract for Spotify ID:", spotifyId, "with payload:", payload);

    const response = await fetch(`${BASE_URL}/artists/${spotifyId}/update-contract`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${jwtToken}`,
        "Content-Type": "application/json",
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Failed to update contract address: ${errorText}`);
    }

    const data = await response.json();
    console.log("[updateArtistContractAddress] Contract address updated successfully:", data);
    return data;
  } catch (error) {
    console.error("[updateArtistContractAddress] Error updating artist contract address:", error);
    throw error;
  }
};
